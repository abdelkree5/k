    ...
    def _get_explainer(self):
        if self._explainer is None:
            try:
                ...
                logger.warning(f"SHAP not available: {e}. Using fallback feature importance.")
    ...
    def shap_values_single(self, X: np.ndarray) -> dict:
        ...
        if self._shap_available:
            ...
            if isinstance(sv, list):
                ...
                values = sv[0]
            result = {
                "shap_values": dict(zip(self.feature_names, [round(float(v), 6) for v in values])),
            ...
            if hasattr(self.model, 'feature_importances_'):
                ...
                result = {
                    "shap_values": dict(zip(self.feature_names, [round(float(v), 6) for v in norm_importances])),
                ...
                result = {
                    "shap_values": dict(zip(self.feature_names, [round(uniform_val, 6)] * len(self.feature_names))),
                    "base_value": 0.5,
                }
