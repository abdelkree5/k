...
class FairnessAnalyzer:
    ...
    def equal_opportunity_diff(self, y_true, y_pred, sensitive, privileged) -> dict:
        ...
        mask = sensitive == privileged
        diff = abs(_tpr(y_true[mask], y_pred[mask]) - _tpr(y_true[~mask], y_pred[~mask]))
    ...
    def demographic_parity(self, y_pred, sensitive, privileged) -> dict:
        ...
        return {"difference": round(float(diff), 4), "compliant": bool(diff < self.threshold)}

    def full_report(self, y_true, y_pred, ages, default_age_threshold: int = 50) -> dict:
        age_group = (ages > default_age_threshold).astype(int)
        report = {
            "age_disparate_impact":      self.disparate_impact(y_pred, age_group, 0),
            "age_equal_opportunity":     self.equal_opportunity_diff(y_true, y_pred, age_group, 0),
